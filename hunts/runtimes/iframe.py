...
class IFrameRuntime():
    def evaluate(self, url, team_puzzle_data, user_puzzle_data, team_data, user_data):
        ...
        return '<iframe width="100%%" onload="resizeIframe(this)" src="%s"></iframe><script>function resizeIframe(obj) { obj.style.height = obj.contentWindow.document.body.scrollHeight + "px"; }</script>' % url
